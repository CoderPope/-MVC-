  ...
  <section id="container" >
      ...
      *********************************************************************************************************************************************************** -->
      ...
      <%
       ...
       ArrayList allStudentList = (ArrayList)request.getAttribute("allStudentList");
       if(allStudentList==null){
       		response.sendRedirect(""+path+"/StudentServlet?operate=studentInfo");
       }
       else{
       		int pageSize = 10;
      ...
      <section id="main-content">
          <section class="wrapper">
          	<h3><i class="fa fa-angle-right"></i> 学生信息管理：</h3>
		  		<div class="row mt">
			  		<div class="col-lg-12">
                          ...
                          <form action="<%=path%>/StudentServlet">
                            <table class="table table-bordered table-striped table-condensed">
                              ...
                              <%
         						...
         						Student studentInfo = (Student)allStudentList.get(i); 
         					  %>
                              <tr>
                              	  ...
                              	  <td class="numeric"><%=studentInfo.getSsex() %></td>
                              	  <td class="numeric"><%=studentInfo.getSage() %></td>
                                  <td class="numeric"><%=studentInfo.getMajor() %></td>                                  
                                  <td class="numeric"><span class="badge bg-theme04"><i class="fa fa-trash-o "><a href="<%=path%>/StudentServlet?operate=delete&Sno=<%=studentInfo.getSno()%>">删除</a></i></span></td>
                              </tr>
                              <%} %>
                                                         
                              </tbody>
                          </table>
                          <span class="btn btn-warning btn-lg"><a href="<%=path%>/StudentServlet?operate=updateQuery">批量修改</a></span>
        	...
        	<table  align="right" width="300" > 
        		<tr >
        			...
        			%>
        					<td ><a href="<%=path%>/StudentServlet?operate=studentInfo&pageN=<%=nowPage-1 %>">上一页</a></td>       			
        			<%}%>     			
        			
        			<%
        				for(int i =1;i<=pageNum;i++){       					
        			%>                                                       
                    <td ><a href="<%=path%>/StudentServlet?operate=studentInfo&pageN=<%=i %>"><%=i %></a></td>
                    <%} %>
        			...
        			%>
        					<td ><a href="<%=path%>/StudentServlet?operate=studentInfo&pageN=<%=nowPage+1 %>">下一页</a></td>       			
        			<%}%>
      ...
      <footer class="site-footer">
          <div class="text-center">
              ...
              <a href=""+path+"/manager/studentInfoManament.jsp#" class="go-top">
